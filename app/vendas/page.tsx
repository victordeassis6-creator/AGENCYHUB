  ...
  return (
    <div className="space-y-10 lg:space-y-14 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 animate-fade-in-up">
        <div>
          <h2 className="text-3xl font-black italic tracking-tighter text-white flex items-center gap-3">
             Novos Planos. <Briefcase className="w-7 h-7 text-indigo-400" />
          </h2>
        ...
        <Button onClick={() => toast("Criar Proposta...")} className="h-12 px-8 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-indigo-500/20">
      ...
      <div className="grid lg:grid-cols-7 gap-8">
        {/* Main List */}
        <Card className="lg:col-span-5 glass border-white/[0.06] rounded-[2.5rem] overflow-hidden shadow-2xl bg-[#111113]">
          <CardHeader className="p-8 border-b border-white/[0.06] flex flex-row items-center justify-between pb-6">
             <div className="flex gap-8">
                <button 
                  onClick={() => setActiveTab("propostas")}
                  className={`text-[10px] font-black uppercase tracking-widest pb-4 border-b-2 transition-all ${activeTab === "propostas" ? "border-indigo-500 text-white" : "border-transparent text-slate-500 hover:text-slate-300"}`}
                >
                  Propostas
                </button>
                <button 
                  onClick={() => setActiveTab("contratos")}
                  className={`text-[10px] font-black uppercase tracking-widest pb-4 border-b-2 transition-all ${activeTab === "contratos" ? "border-indigo-500 text-white" : "border-transparent text-slate-500 hover:text-slate-300"}`}
                >
                  Contratos
                </button>
             </div>
              ...
              {proposals.map((item) => (
                <div key={item.id} className="p-6 flex items-center justify-between hover:bg-white/[0.04] transition-all rounded-3xl group cursor-pointer">
                  <div className="flex items-center gap-6">
                    <div className="h-14 w-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 group-hover:text-indigo-400 group-hover:border-indigo-500/30 transition-all">
        ...
        <div className="lg:col-span-2 space-y-8">
           <Card className="glass border-indigo-500/20 rounded-[2.5rem] bg-gradient-to-br from-indigo-500/10 to-purple-500/10 overflow-hidden shadow-3xl">
              <CardHeader className="p-8 pb-4">
                 <CardTitle className="text-white text-xl font-black italic flex items-center gap-2 tracking-tighter">
                    <Sparkles className="w-6 h-6 text-indigo-400" /> Proposta Rápida
                 </CardTitle>
              ...
              <CardContent className="p-8 space-y-6">
                 ...
                 <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       <Button size="sm" variant="outline" className="h-10 rounded-xl text-[10px] font-black uppercase tracking-widest bg-indigo-500/10 border-indigo-500/40 text-indigo-300">Plano Pro</Button>
           ...
           <Card className="glass border-white/5 rounded-[2.5rem] bg-[#111113] p-8 shadow-2xl overflow-hidden relative">
